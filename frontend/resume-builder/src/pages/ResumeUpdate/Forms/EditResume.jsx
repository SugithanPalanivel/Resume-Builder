...
import EducationDeatilsForm from './Forms/EducationDeatilsForm';
...
import { fixTailwindColors,dataURLtoFile,captureElementAsImage } from '../../../utils/helper';
...
const EditResume = () => {
  ...
  const [openThemeSlector,setOpenThemeSelector]=useState(false);
      ...
      case "education-info":
        resumeData.education.forEach(
          ({degree,institution,startDate,endDate},index)=>{
            ...
            if(!institution.trim())
              errors.push(`Instituition is required in education ${index + 1}`);
            if(!startDate || !endDate)
          ...
          case "work-experience":
            return(
              <WorkExperienceForm
                ...
                romoveArrayItem={(index)=>removeArrayItem("workExperience",index)}
                
              />
            );
            case "education-info":
              return(
                <EducationDeatilsForm
  ...
  // update simple nested object (like profileInfo,contactInfo)
  ...
  // Update Array items(like workExperience[0],skills:[1])
  const updateArrayItem=(section,index,key,value)=>{
    setResumeData((prev)=>{
      const updatedArray=[...prev[section]];

      if(key === null){
        updatedArray[index]=value; //for simple strings like intersets
      }
      else{
        updatedArray[index]={
          ...updatedArray[index],
  ...
  try {
    ...
    // console.log("imageDataUrl:", imageDataUrl); // Debug
    ...
    // console.log("profileImageFile:", profileImageFile); // Debug
    // console.log("thumbnailFile:", thumbnailFile); // Debug
    ...
    for (let pair of formData.entries()) {
      console.log(pair[0], pair[1]); // Debug log for FormData
    }
                    ...
                    <button
                      ...
                      >
                        ...
                        {currentPage !== "additionalInfo" && (
                          <LuArrowLeft className='text-[16px] rotate-100'/>
                        )}
                      </button>
              </div>
             </div>
